...
keystone endpoint-create --service-id=$(keystone service-list | awk '/ object-store / {print $2}') --publicurl='http://$CON_MGNT_IP:8080/v1/AUTH_%(tenant_id)s' --internalurl='http://$CON_MGNT_IP:8080/v1/AUTH_%(tenant_id)s' --adminurl=http://$CON_MGNT_IP:8080
